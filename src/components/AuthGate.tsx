...
// Blocks the whole app until the visitor signs in (email/password now, Google
// once Supabase is configured).
export function AuthGate({ children }: { children: ReactNode }) {
  const { user, loading, googleEnabled, signUp, signIn, signInWithGoogle } = useAuth()
    ...
    return (
      <div className="gate">
        <div className="gate-card">
          ...
          <p className="fine-print">Checking your session…</p>
        </div>
      </div>
    )
  }
      ...
      <div className="gate-card card">
        ...
        {!googleEnabled && (
          <p className="form-note" style={{ marginTop: 8 }}>
            Google sign-in setup pending — use email for now.
          </p>
        )}
